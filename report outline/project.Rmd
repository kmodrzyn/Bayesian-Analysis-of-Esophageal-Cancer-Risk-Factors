...
#pp_dens_2_model_1 + xaxis_title() + ggplot2::xlab("Model 1 grouped by age")

```

```{r}
# Combine the plots
grid.arrange(pp_dens_1_model_1, pp_dens_2_model_1, ncol = 2)
```
...
```{r}
pp_check(model, type="bars", newdata = test_data)
```

```{r}
pp_check(model, type="bars_grouped", group="age", newdata = test_data)
```

The final two plots show the observed mean statistic and a distritbution of the statistic computed from the simulated data set.

```{r}
pp_check(model, type="stat", newdata = test_data)
```

```{r}
pp_check(model, type="stat_grouped", group="age", newdata = test_data)
```
...
We can then construct a confusion matrix by comparing predictions to the actual outcomes in the test set.
...
```{r}
conf_matrix_df_model1 <- as.data.frame(as.table(conf_matrix_model1))

# Rename the columns appropriately
names(conf_matrix_df_model1) <- c("Predicted", "Actual", "Frequency")

ggplot(conf_matrix_df_model1, aes(x = Predicted, y = Actual, fill = Frequency)) +
    geom_tile() +
...
In the hierarchical model, we will assume that each patient will inherently have a distinct tendency to have cancer. For example, depending on the patient's genes, the patient is either vulnerable or more resistant to cancer.

### Priors and Likelihood:

1.  **Priors**: We can use the flat priors for the coefficients and Student - T prior for each patient suggested by BRMS and then later test the prior sensitivity of the model.
...
```{r}
pp_check(model2, newdata = test_data, allow_new_levels=TRUE, sample_new_levels="gaussian")
```

```{r}
pp_check(model2, type="dens_overlay_grouped", group="age", newdata = test_data, allow_new_levels=TRUE, sample_new_levels="gaussian")
```

The next two plots displays two bars representing the observed counts of the two outcomes, with overlaid bars from the simulated data.

```{r}
pp_check(model2, type="bars_grouped", group="age", newdata = test_data, allow_new_levels=TRUE, sample_new_levels="gaussian")
```
...
```{r}
pp_check(model2, type="stat_grouped", group="age", newdata = test_data, allow_new_levels=TRUE, sample_new_levels="gaussian")
```
...
```{r}
# Rename the columns appropriately
names(conf_matrix_df_model2) <- c("Predicted", "Actual", "Frequency")

ggplot(conf_matrix_df_model2, aes(x = Predicted, y = Actual, fill = Frequency)) +
    geom_tile() +
    scale_fill_gradient(low = "white", high = "steelblue") +
    theme_minimal() +
    labs(x = "Predicted", y = "Actual", fill = "Count") +
